...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_official_bulletin_type" AS ENUM('disposiciones', 'resoluciones', 'ordenanzas');
  CREATE TYPE "public"."enum_commitment_status" AS ENUM('not-started', 'in-progress', 'completed');
  CREATE TYPE "public"."enum_participation_project_area" AS ENUM('educacion', 'salud');
  CREATE TYPE "public"."enum_participation_status" AS ENUM('pendiente', 'en-revision', 'aprobado', 'rechazado');
  ...
  CREATE TYPE "public"."enum_home_customization_enabled_sections" AS ENUM('organigrama_person', 'official_bulletin', 'commitment', 'budget', 'tenders', 'public-works', 'participation');
  ...
  CREATE TABLE "organigrama_person_subordinate_areas" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  	"area_name" varchar,
  ...
  CREATE TABLE "official_bulletin" (
  	...
  	"type" "enum_official_bulletin_type",
  	"hero_text" varchar,
  ...
  CREATE TABLE "participation" (
  	...
  	"hero_text" varchar,
  	"instructions" jsonb,
  	"enable_submissions" boolean DEFAULT true,
  	"title" varchar,
  	"full_name" varchar,
  	"email" varchar,
  	"project_area" "enum_participation_project_area",
  	"summary" varchar,
  	"status" "enum_participation_status" DEFAULT 'pendiente',
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  CREATE TABLE "home_customization_enabled_sections" (
  	"order" integer NOT NULL,
  	"parent_id" integer NOT NULL,
  	"value" "enum_home_customization_enabled_sections",
  	"id" serial PRIMARY KEY NOT NULL
  );
  ...
  ALTER TABLE "organigrama_person_subordinate_areas" ADD CONSTRAINT "organigrama_person_subordinate_areas_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."organigrama_person"("id") ON DELETE cascade ON UPDATE no action;
  ...
  ALTER TABLE "commitment" ADD CONSTRAINT "commitment_image_id_media_id_fk" FOREIGN KEY ("image_id") REFERENCES "public"."media"("id") ON DELETE set null ON UPDATE no action;
  ALTER TABLE "budget" ADD CONSTRAINT "budget_files_id_media_id_fk" FOREIGN KEY ("files_id") REFERENCES "public"."media"("id") ON DELETE set null ON UPDATE no action;
  ...
  ALTER TABLE "home_customization_enabled_sections" ADD CONSTRAINT "home_customization_enabled_sections_parent_fk" FOREIGN KEY ("parent_id") REFERENCES "public"."home_customization"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE INDEX "organigrama_person_subordinate_areas_order_idx" ON "organigrama_person_subordinate_areas" USING btree ("_order");
  CREATE INDEX "organigrama_person_subordinate_areas_parent_id_idx" ON "organigrama_person_subordinate_areas" USING btree ("_parent_id");
  ...
  CREATE UNIQUE INDEX "official_bulletin_number_idx" ON "official_bulletin" USING btree ("number");
  ...
  CREATE INDEX "commitment_image_idx" ON "commitment" USING btree ("image_id");
  ...
  CREATE INDEX "budget_files_idx" ON "budget" USING btree ("files_id");
  ...
  CREATE INDEX "home_customization_enabled_sections_order_idx" ON "home_customization_enabled_sections" USING btree ("order");
  CREATE INDEX "home_customization_enabled_sections_parent_idx" ON "home_customization_enabled_sections" USING btree ("parent_id");`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "organigrama_person_subordinate_areas" CASCADE;
  ...
  DROP TABLE "home_customization_enabled_sections" CASCADE;
  DROP TABLE "home_customization" CASCADE;
  DROP TYPE "public"."enum_users_role";
  DROP TYPE "public"."enum_official_bulletin_type";
  DROP TYPE "public"."enum_commitment_status";
  DROP TYPE "public"."enum_participation_project_area";
  DROP TYPE "public"."enum_participation_status";
  DROP TYPE "public"."enum_public_works_work_type";
  DROP TYPE "public"."enum_home_customization_enabled_sections";`)
}
